...
class ProviderKeyTestResponse(BaseModel):
    """Provider API key test response."""

    success: bool = Field(..., description="Whether the key test succeeded.")
    provider: str = Field(..., description="Provider name used for the test.")
    model: str = Field(..., description="Model name used for the test.")
    message: str = Field(..., description="Test result message.")


class WebsiteBuilderRequest(BaseModel):
    """AI website builder request routed through the backend."""

    provider: str = Field(..., description="Provider name, e.g. OpenAI or Google.")
    model: str = Field(..., description="Model name to call.")
    api_key: str = Field(..., description="API key for the selected model.")
    prompt: str = Field(..., description="Prompt for website generation.")
    base_url: Optional[str] = Field(default=None, description="Optional custom base URL.")


class WebsiteBuilderResponse(BaseModel):
    """AI website builder response."""

    content: str = Field(..., description="Generated response text.")
    provider: str = Field(..., description="Provider name used for the request.")
    model: str = Field(..., description="Model name used for the request.")


class AssignedAppRunRequest(BaseModel):
    """Run an HQ-assigned AI app through backend provider routing."""

    app_key: str = Field(..., description="Assigned AI app key, e.g. ai-chat.")
    prompt: str = Field(..., description="Prompt or user question to send.")
    history: Optional[List[ChatMessage]] = Field(default=None, description="Optional recent conversation history.")
    site_id: Optional[str] = Field(default=None, description="Optional published site id for plan/client/agency scope resolution.")
    project_id: Optional[int] = Field(default=None, description="Optional project id for plan/client scope resolution.")
    org_id: Optional[int] = Field(default=None, description="Optional organization id for direct scope resolution.")


class AssignedAppRunResponse(BaseModel):
    """Assigned AI app run response."""

    content: str = Field(..., description="Generated response text.")
    provider: str = Field(..., description="Resolved provider key used for the request.")
    model: str = Field(..., description="Resolved model name used for the request.")
    app_key: str = Field(..., description="Assigned AI app key that handled the request.")
...
class GenAudioRequest(BaseModel):
    ...
    voice_style_key: Optional[str] = Field(
        default=None,
        description="Optional frontend voice style key, used to keep the selected male/female preset consistent.",
    )
