...
SENSITIVE_KEY_PARTS = ("password", "secret", "token", "api_key", "private_key", "database_url", "mask_key")
...
def require_local_setting_mutation() -> None:
    if not settings.is_development_environment:
        raise HTTPException(status_code=403, detail="Environment values are managed by the production secret manager")


def reject_sensitive_frontend_key(key: str) -> None:
    if is_sensitive_key(key):
        raise HTTPException(status_code=400, detail="Sensitive values must not be written to frontend environment files")
...
async def get_settings(current_user: UserResponse = Depends(get_admin_user)):
    ...
    try:
        ...
        for key, value in backend_vars.items():
            backend_config[key] = EnvVariable(key=key, value="[configured]" if is_sensitive_key(key) and value else value, description=backend_descriptions.get(key, ""))

        frontend_config = {}
        for key, value in frontend_vars.items():
            frontend_config[key] = EnvVariable(key=key, value="[configured]" if is_sensitive_key(key) and value else value, description=frontend_descriptions.get(key, ""))
...
async def update_frontend_setting(
    ...
    try:
        require_local_setting_mutation()
        reject_sensitive_frontend_key(key)
