...
class DatabaseManager:
    ...
    async def init_db(self):
        ...
        try:
            ...
            if database_url.startswith("sqlite+aiosqlite"):
                # SQLite is supported only for local development.  The local
                # launcher, Vite proxy and background verification can still
                # briefly overlap, so wait for a writer instead of failing a
                # request immediately with "database is locked". Production
                # uses PostgreSQL and is intentionally not affected.
                engine_kwargs["connect_args"] = {"timeout": 30}
                logger.info("Using SQLite async engine options with a 30-second busy timeout")
            elif is_lambda:
    ...
    async def create_tables(self):
        """Create all tables for an explicitly local development bootstrap only."""
        if not settings.should_bootstrap_database_schema:
            raise RuntimeError(
                "Automatic table creation is disabled outside dev/local/test. "
                "Run 'alembic upgrade head' before starting this process."
            )
    ...
    async def verify_migration_state(self):
        """Fail fast when a non-development database was not migrated explicitly."""
        if not self.engine:
            raise RuntimeError("Database engine not initialized")
        ...
        try:
            async with self.engine.connect() as conn:
                ...
                applied_revisions = {str(row[0]) for row in result.fetchall()}
        except SQLAlchemyError as exc:
            raise RuntimeError(
                "Database migration state is missing. Run 'alembic upgrade head' before starting the application."
            ) from exc

        if not applied_revisions:
            raise RuntimeError("Database migration state is empty. Run 'alembic upgrade head' before starting the application.")
        ...
        try:
            ...
            expected_heads = set(ScriptDirectory.from_config(Config(str(alembic_ini))).get_heads())
        except Exception as exc:
            raise RuntimeError("Unable to determine the required Alembic migration head") from exc

        if applied_revisions != expected_heads:
            raise RuntimeError(
                "Database migration is not current. "
                f"Applied={sorted(applied_revisions)}, required={sorted(expected_heads)}. "
                "Run 'alembic upgrade head' before starting the application."
            )

        logger.info("Database migration state verified at %s", sorted(expected_heads))

    async def initialize_runtime_schema(self):
        """Select the safe schema path for this runtime environment."""
        if settings.should_bootstrap_database_schema:
            logger.warning("Development schema bootstrap is enabled; never use this mode in production")
            await self.create_tables()
            return

        logger.info("Migration-only schema mode enabled; automatic DDL and repair are disabled")
        await self.verify_migration_state()
    ...
    async def ensure_initialized(self):
        ...
        try:
            await self.init_db()
            await self.initialize_runtime_schema()
